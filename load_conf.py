...
gsflag = 0
ssflag = 0
WhichBaseLinesGlobal = 4
# 001(1)->select GtoG 010(2)->SELECT GtoS, 100(4)->StoS

# 观测频率和带宽
ObservFreqGlobal = 5e9
BandWidthGlobal = 3.2e7

# 单位选择标志
unitflag = 1

# cutoffmode=1 #截止模式选择
cutoffmode = {'flag': 0, 'CutAngle': 10}  # 截止模式选择，flag:0->取数据库中设置的水平角，1->取界面上设置的水平角 2->取大者，3->取小者
precessionmode = {'flag': 0}  # 进动模型选择，0->Two-Body,1->J2

###########################################
# 3. 源，观测站，卫星的信息
###########################################
# 源信息
SouGllst = [['0134+329', 0.42624576, 0.57874696]]
# VLBI站信息
VLBGllst = [['Effelsberg', 4033.94775, 486.99037, 4900.430, 20],
...
# 遥测站信息
TeleGllst = [['Goldstone', -2353.62000, -4641.34000, 3677.05000]]
# 卫星列表信息,每一元祖数据对应的信息为Name,a,e,i,w,Ω,M0,Epoch
SatGllst = [['VSOP', 17367.457, 0.60150, 31.460, 106.755, 16.044, 66.210, 50868.00000],
...
def print_setting():
    ...
    label4 = "Wavelength: %f" % (light_speed*100/ObservFreqGlobal)
    info.append(label4 + '\n')

    label5 = "Source:\n\t"
    for item in SouGllst:
    ...
    label6 = "Satellite:\n\t"
    for item in SatGllst:
       label6 = label6+item[0]
       label6 = label6+'\n\t'
    info.append(label6)

    label7 = 'VLBI Stations:\n\t'
    for item in VLBGllst:

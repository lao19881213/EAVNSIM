...
def obs_all_active_sta(time_mjd, ra_src, dec_src, pos_mat_sat, pos_mat_telemetry,
                       pos_mat_vlbi, baseline_type, cutoff_mode, precession_mode):
    """
    输入卫星天球坐标系
    :param time_mjd: 观测时间
    :param ra_src:   源的位置
...
def obs_judge_active_satellite(time_mjd, pos_vec_sat, pos_vec_telemetry, ra_src, dec_src, cutoff_dict):
    ...
    :param pos_vec_telemetry:  遥测站的位置
    :param ra_src:  源的信息
    :param dec_src:
    ...
    for i in np.arange(len(pos_vec_telemetry)):
        telemetry_visibility_result.extend([pos_vec_telemetry[i][0], False])
    ...
    for i in np.arange(len(pos_vec_telemetry)):
        ...
        long_telemetry, lat_telemetry, height_telemetry = tc.itrf_2_geographic(pos_vec_telemetry[0][1],
                                                                               pos_vec_telemetry[0][2],
                                                                               pos_vec_telemetry[0][3])
...
def obs_judge_active_vlbi_station(ra_src, dec_src, time_mjd, long_vlbi_sta, lat_vlbi_sta, horizon_vlbi_sta):
    ...
    :return: yes / no
    """
    source_azimuth, source_elevation = tc.equatorial_2_horizontal(time_mjd, ra_src, dec_src,
...
def obs_satellite_to_source(ra_src, dec_src, time_mjd, pos_sat_x, pos_sat_y, pos_sat_z):
    ...
    # 计算黄赤交角
    epsilon = tt.ecliptic_obliquity(time_mjd)
    # 将源赤道单位向量转化为3x1矩阵
    src_equ = np.array([[src_equ_x], [src_equ_y], [src_equ_z]])
    # 计算源黄道单位矢量，返回类型为3X1矩阵
    src_ecliptic = tc.equatorial_2_ecliptic(src_equ, epsilon)
    ...
    # 检查约束？
    angle = tu.angle_btw_vec(src_ecliptic, sun_ecliptic)  # Angle的范围是多少 0~pi
    ...
    if angle < np.pi * 7 / 18:
        ...
        angle = tu.angle_btw_vec(src_ecliptic, moon_ecliptic)
        # 检查月球约束
        if angle < np.pi / 9:
            visibility = False
        else:
...
def obs_telemetry_to_satellite(pos_x_sat, pos_y_sat, pos_z_sat, long_telemetry,
    ...
    sat_az_el = tc.itrf_2_horizontal(sat_lst, long_telemetry, lat_telemetry, height_telemetry)
    azimuth = sat_az_el[0]

...
def test_single_uv(fig_handle):
    ...
    if (lc.baseline_type & 1) != 0:
        ...
        if x is not None and y is not None:
            x = np.array(x)
            y = np.array(y)
            fig_handle.scatter(x, y, s=5, marker='.', color='b')
    ...
    if max_u > 0 or max_v > 0:
        ...
        fig_handle.set_xlim(-max_range, +max_range)
        fig_handle.set_ylim(-max_range, +max_range)

    fig_handle.set_title("UV Plot")
    if lc.unit_flag == 0:
        fig_handle.set_xlabel("u$(\lambda)$")
        fig_handle.set_ylabel("v$(\lambda)$")
    else:
        fig_handle.set_xlabel("u$(m)$")
        fig_handle.set_ylabel("v$(m)$")
    fig_handle.grid()


def test_sky_uv(fig_handle):
    # load data from configurations
    start_time = tt.time_2_mjd(lc.StartTimeGlobalYear, lc.StartTimeGlobalMonth,
                               lc.StartTimeGlobalDay, lc.StartTimeGlobalHour,
                               lc.StartTimeGlobalMinute, lc.StartTimeGlobalSecond, 0)
    ...
    for i in (2, 6, 10, 14, 18, 22):
        for j in (-60, -30, 0, 30, 60):
            if len(result_mat_u[k]) > 0 and len(result_mat_v[k]) > 0:
                ...
                temp_u += i
                temp_v += j
                fig_handle.scatter(temp_u, temp_v, s=3, marker='.', color='b')
            k += 1
    # plot sun position
    sun_ra, sun_dec = me.sun_ra_dec_cal(start_time, stop_time, time_step)
    fig_handle.plot(np.array(sun_ra), np.array(sun_dec), '.k', linewidth=2)
    fig_handle.plot(sun_ra[0], sun_dec[0], 'or', alpha=0.5, markersize=20)
    # ticks
    fig_handle.set_title("All Sky UV Plot")
    fig_handle.set_xlabel(r"Ra($H$)")
    fig_handle.set_ylabel(r'Dec ($^\circ$)')
    fig_handle.set_xticks([0, 2, 6, 10, 14, 18, 22, 24])
    fig_handle.set_yticks([-90, -60, -30, 0, 30, 60, 90])
    fig_handle.set_xlim(0, 24)
    fig_handle.set_ylim(-90, +90)
    # fig_handle.set_gid('line')
...
def main(wins=None):
    ...
    # figUV = Figure(figsize=(15, 8)) # 如果figsize 比例设置的不好，则有部分内容无法显示
    ...
    # test_plot(UVPlot)
    UVPlot.cla()
    test_single_uv(UVPlot)

    # SKYUV plots
    # SKYUVPlot = figUV.add_subplot(122, aspect='equal')
    SKYUVPlot = figUV.add_subplot(122, aspect=0.14)  # width/height
    SKYUVPlot.cla()
    test_sky_uv(SKYUVPlot)

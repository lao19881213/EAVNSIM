...
_github_page_ = "https://github.com/ZhenZHAO/EAVNSIM"
__about_text__ = """ 
   EAVNSIM,  AN INTEGRATED VLBI SIMULATOR FOR EAST ASIA VLBI NETWORK

                     ZHEN ZHAO
  (Shanghai Astronomical Observatory)

EAVNSIM, an integrated VLBI simulator for East Asia VLBI Network(EAVN), aims to assist radio astronomers to design, evaluate and simulate VLBI experiments in a friendly, flexible, portable platform. That integrates the functions of SCHED, EVN calculator and part of DIFMAP. 
...
"""
__help_text__ = """ 
EAVNSIM is an integrated VLBI simulator for East Asia VLBI Network. You can config your specific simulation on the left configure panel, where we support space VLBI and various constraints. 
...
class AppData(object):
    ...
    def update_calculation_class(self):
        ...
        self.myFuncImg = FuncImg(self.var_model_name, self.var_pix_size,
                                 ...
                                 True, self.var_clean_gain, self.var_clean_thresh, self.var_clean_niter)
    ...
    def update_data_img(self):
        if len(self.result_uv_single_u) > 0 and len(self.result_uv_single_v) > 0 and self.var_model_name != '':
            ...
            tmp_clean, tmp_res = self.myFuncImg.get_result_clean_map_with_update()
...
class AppGUI(object):
    ...
    def _init_ui_choice_para(self):
        ...
        self.ui_choice_color_map = ['jet', 'rainbow', 'Greys', 'hot', 'cool', 'nipy_spectral']
    ...
    def _load_ui_para_to_cal_para(self):
        ...
        self.myData.var_unit = self.ui_choice_unit.index(self.ui_unit.get())
        # print(self.var_unit)
    ...
    def _gui_int(self):
        ...
        # 1.1. EAVNSIM (2 dialog)
        menu_sim = tk.Menu(menu, tearoff=True)
        menu.add_cascade(label="EAVNSIM", menu=menu_sim)
    ...
    def update_panel_img(self):
        ...
        # draw new - src
        show_model, x_max = self.myData.get_data_img_model()
        model_plot = self.figs_img_fig_src.imshow(np.power(show_model, gamma), picker=True,
                                                  ...
                                                  cmap=colormap)
        pl.setp(model_plot, extent=(x_max / 2., -x_max / 2., -x_max / 2., x_max / 2.))

        # draw new - beam
        show_beam, x_max = self.myData.get_data_img_beam()
        beam_plot = self.figs_img_fig_beam.imshow(show_beam, picker=True, interpolation='nearest', cmap=colormap)
        pl.setp(beam_plot, extent=(x_max / 2., -x_max / 2., -x_max / 2., x_max / 2.))

        # draw new - dirty
        show_map, x_max = self.myData.get_data_img_dirty()
        dirty_plot = self.figs_img_fig_dirty.imshow(show_map, picker=True, interpolation='nearest', cmap=colormap)
        pl.setp(dirty_plot, extent=(x_max / 2., -x_max / 2., -x_max / 2., x_max / 2.))

        # draw new - clean
        clean_img, res_img = self.myData.get_data_img_clean()
        clean_plot = self.figs_img_fig_clean.imshow(clean_img, picker=True, interpolation='nearest', cmap=colormap)
        pl.setp(clean_plot, extent=(x_max / 2., -x_max / 2., -x_max / 2., x_max / 2.))
    ...
    def _gui_ini(self):
        ...
        if self.show_type == UV_SHOW_POP_TYPE_YEAR:
            ...
            if len(result_time_u) != 0 and len(result_time_v) != 0:
                ...
                my_plot.set_xticks([4, 12, 20], [1, 2, 3])
                my_plot.set_yticks([3, 9, 15, 21], [4, 3, 2, 1])
            ...
            if num_src > 0:
                ...
                for k in range(num_src):
                    ...
                    # print(tmp_pos)
                    tmp_plot = fig.add_subplot(tmp_pos, aspect='equal')

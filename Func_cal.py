...
class ParaCal(object):
    def __init__(self, window=None):
        ...
        for teleStation in self.teleDict:
            ...
            tk.Checkbutton(config_left_frm, text=teleStation, variable=self.teleDict[teleStation],
        ...
        ttk.Combobox(config_right_frm, textvariable=self.waveband, values=self.obevBandList, width='12').grid(row=0, column=1)

        self.datarate = tk.StringVar(window, self.datarateList[1])
        tk.Label(config_right_frm, text='Data Rate:').grid(row=0, column=2)
        ttk.Combobox(config_right_frm, textvariable=self.datarate, values=self.datarateList, width='18').grid(row=0, column=3)
        ...
        ttk.Combobox(config_right_frm, textvariable=self.specChNum, values=self.channelNumberList, width='12').grid(row=1, column=1)

        self.integrateTime = tk.StringVar(window, '2 s')
        tk.Label(config_right_frm, text='Integrate Time:').grid(row=1, column=2)
        ttk.Combobox(config_right_frm, textvariable=self.integrateTime, values=self.integrateTimeList, width='18').grid(row=1, column=3)
        ...
        ttk.Combobox(config_right_frm, textvariable=self.polarNum, values=self.polarNumberList, width='12').grid(row=2, column=1)

        self.subBandBandwidth = tk.StringVar(window, '16 MHz')
        tk.Label(config_right_frm, text='Subband BW:').grid(row=2, column=2)
        ttk.Combobox(config_right_frm, textvariable=self.subBandBandwidth, values=self.subBandBandwidthList, width='18').grid(row=2, column=3)
        ...
        ttk.Combobox(config_right_frm, textvariable=self.subBandNum, values=self.subBandNumberList, width='12').grid(row=3,
                                                                                                           column=1)

        self.baselineLen = tk.StringVar(window, '10000 km (Full EVN)')
        tk.Label(config_right_frm, text='Baseline Len:').grid(row=3, column=2)
        ttk.Combobox(config_right_frm, textvariable=self.baselineLen, values=self.baselineLenList, width='18').grid(row=3,
                                                                                                          column=3)
        ...
        self.noiseOutWin = tk.Label(frm_result_top, textvariable=self.thermalNoise, width=20, bg='lightblue', fg='white')
    ...
    def capacity_calculation(self):
        ...
        if self.parNum * self.subNum > 16:
            ...
            corr_usage_1 = (self.stationNum * self.stationNum * self.parNum * self.crossNum * self.subNum * self.specNum) / (131072.0)
